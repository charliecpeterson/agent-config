...
PERSONAL_MCPS=(
  ...
  "comfyui_mcp|https://github.com/charliecpeterson/comfyui_mcp.git|comfyui-mcp"
...
env_args=()
[[ -f "$path/.env" ]] && env_args=(--env-file "$path/.env")
exec uv --directory "$path" run$extra_flags "\${env_args[@]}" $entrypoint "\$@"
EOF
  chmod +x "$wrapper"
